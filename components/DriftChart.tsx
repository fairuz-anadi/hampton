    ...
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" role="img"
      ...
      {ticks.map((t) => (
        <g key={t}>
          ...
          <text x={L - 10} y={y(t) + 4} textAnchor="end" fill="var(--faint)" fontFamily="IBM Plex Mono, monospace" fontSize={22}>
            {t > 0 ? `+${t}` : t}
          </text>
        </g>
      ))}
      ...
      <text x={x(splitAt + 0.5) + 7} y={T + 11} fill="var(--critical)" fontFamily="IBM Plex Mono, monospace" fontSize={22}>
        script {splitAt}
      </text>
      ...
      <text x={x(1)} y={y(earlyMean) - 10} fill="var(--good)" fontFamily="IBM Plex Mono, monospace" fontSize={26} fontWeight={600}>
        mean +{earlyMean.toFixed(2)}
      </text>
      <text x={x(totalScripts)} y={y(lateMean) + 18} textAnchor="end" fill="var(--faculty)" fontFamily="IBM Plex Mono, monospace" fontSize={26} fontWeight={600}>
        mean +{lateMean.toFixed(2)}
      </text>

      <line x1={L} y1={B} x2={R} y2={B} stroke="var(--rule-strong)" strokeWidth={1} />
      {[1, splitAt, totalScripts].map((n) => (
        <text key={n} x={x(n)} y={B + 18} textAnchor="middle" fill="var(--faint)" fontFamily="IBM Plex Mono, monospace" fontSize={22}>
          {n}
        </text>
      ))}
      <text x={(L + R) / 2} y={B + 38} textAnchor="middle" fill="var(--faint)" fontFamily="IBM Plex Mono, monospace" fontSize={22} letterSpacing="0.12em">
        MARKING ORDER
      </text>
      <text
        x={14}
