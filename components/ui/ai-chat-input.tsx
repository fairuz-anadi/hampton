...
function MorphingText({ text }: { text: string }) {
  ...
  return (
    ...
    >
      ...
      <span key={text} className="absolute inset-0 flex items-center justify-center whitespace-nowrap">
        {text}
      </span>
    </span>
  );
  ...
  (
    {
      ...
      className,
      models = ["This session", "Whole cohort"],
      efforts = ["Low", "Medium", "Max Effort"],
      defaultValue = "",
      value: controlledValue,
      onChange,
      maxAttachments = 6,
      widths = { collapsed: 340, expanded: 560 },
    },
    ref
  ) => {
    ...
    const startRecording = useCallback(async () => {
      ...
      try {
        ...
        // No microphone. Fall through to the simulated path below.
      }

      setIsRecording(true);

      function simulateText() {
        const words = VOICE_DEMO_TEXT.split(" ");
        let i = 0;
    ...
    const handleBlur = (e: React.FocusEvent<HTMLDivElement>) => {
      if (internalContainerRef.current?.contains(e.relatedTarget as Node)) return;
    ...
    return (
      <div
        ref={(node) => {
          ...
          // Mirror the forwarded ref internally so blur/outside-click checks
        ...
        style={{
          maxWidth: expanded ? widths.expanded : widths.collapsed,
        ...
        >
          <style
            dangerouslySetInnerHTML={{
              ...
              .prompt-scrollbar:hover::-webkit-scrollbar-thumb { background: rgba(17,17,17,0.18); }
            `,
            }}
          />
          ...
          <textarea
            ...
            aria-label="Ask about this marking session"
          ...
          >
            <div className="relative">
              <button
                type="button"
                onMouseDown={(e) => e.preventDefault()}
                onClick={(e) => { e.stopPropagation(); setIsModelSelectOpen((p) => !p); }}
                className={cn(
                ...
                aria-label={`Scope. Current: ${selectedModel}`}
              >
